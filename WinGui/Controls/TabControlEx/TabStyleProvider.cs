...
namespace CustomControls 
{
	...
	public abstract class TabStyleProvider : Component
	{
        ...
        protected TabControlEx TabControl { get; private set; }
        ...
        public Point Padding {
			...
			set {
				...
				if (this._ShowTabCloser){
					if (value.X + (int)(this._Radius/2) < -TabControlEx.TabCloserButtonSize){
						((TabControl)this.TabControl).Padding = new Point(0, value.Y);
					} else {
                        ((TabControl)this.TabControl).Padding = new Point(value.X + this._Radius + (int)(TabControlEx.TabCloserButtonSize + 10) / 2, value.Y);
					}
				} else {
